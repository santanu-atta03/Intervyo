...
class EmotionAnalysisController {
  ...
  async storeEmotionMetrics(req, res) {
    try {
      ...
      if (interview.metrics.emotionTimeline.length > 1000) {
        interview.metrics.emotionTimeline =
          interview.metrics.emotionTimeline.slice(-1000);
      }
      if (interview.metrics.confidence.length > 1000) {
        interview.metrics.confidence =
          interview.metrics.confidence.slice(-1000);
      }
  ...
  async getEmotionSummary(req, res) {
    try {
      ...
      emotionTimeline.forEach((entry) => {
        Object.keys(entry.emotions || {}).forEach((emotion) => {
          ...
          if (!emotionCounts[emotion])
            emotionCounts[emotion] = { count: 0, sum: 0 };
      ...
      const avgConfidence =
        avgConfidenceRaw > 1 ? avgConfidenceRaw / 100 : avgConfidenceRaw;
      const maxConfidence =
  ...
  async generateEmotionFeedback(req, res) {
    try {
      ...
      emotionTimeline.forEach((entry) => {
        Object.keys(entry.emotions || {}).forEach((emotion) => {
          emotionCounts[emotion] =
            (emotionCounts[emotion] || 0) + (entry.emotions[emotion] || 0);
        });
      });
      ...
      const feedbackMap = {
        ...
        nervous:
          "You showed some nervousness, which is completely normal. Practice will help you feel more comfortable.",
        fearful:
          "Consider practicing more to build confidence in answering questions.",
      ...
      const confidenceValues = (interview.metrics.confidence || []).map(
        (c) => c.value || 0,
      );
      const avgConfidenceRaw =
        confidenceValues.length > 0
      ...
      const confidenceFeedback = {
        "very-confident":
          "Your speech patterns showed high confidence and clarity.",
        confident:
          "Your speech demonstrated good confidence with minimal hesitations.",
        neutral:
          "Your confidence was average. Working on reducing filler words will help.",
        nervous:
          "You used several filler words and hesitations. Practice speaking clearly and confidently.",
        "very-nervous":
          "Focus on breathing exercises and practicing aloud to improve your confidence.",
      }[confidenceLevel];

      // Store feedback in results
      if (!interview.results) interview.results = {};
      if (!interview.results.detailedFeedback)
        interview.results.detailedFeedback = {};
