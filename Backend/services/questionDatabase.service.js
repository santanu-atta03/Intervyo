...
export const getQuestionFrequency = async (company) => {
  try {
    ...
    const frequencyDistribution = {
      ...
      common: questions.filter((q) => q.timesAsked >= 5 && q.timesAsked < 10)
        .length,
      occasional: questions.filter((q) => q.timesAsked >= 2 && q.timesAsked < 5)
        .length,
...
export const getUserQuestions = async (userId) => {
  try {
    const questions = await RealQuestion.find({ submittedBy: userId }).sort({
      createdAt: -1,
    });
