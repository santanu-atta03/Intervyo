...
export const findBuddies = async (userId, preferences = {}) => {
  try {
    ...
    const connectedUserIds = existingMatches.map((m) =>
      m.user1.toString() === userId.toString()
        ? m.user2.toString()
        : m.user1.toString(),
    );
    ...
    const buddies = potentialBuddies
      ...
      .map((buddy) => {
        ...
        const commonCompanies = buddy.companies.filter((c) =>
          targetCompanies.includes(c),
        );
        const companyScore =
          (commonCompanies.length / targetCompanies.length) * 50;

        // Skill level similarity (prefer similar skill levels)
        const userAvgScore =
          userInterviews.reduce(
            (sum, i) => sum + (i.result?.overallScore || 0),
            0,
