 ...
 */
export const calculateCompanyFit = async (
  user,
  companyName,
  userInterviews,
) => {
  try {
    ...
    const avgSystemDesign =
      systemDesignScores.length > 0
        ? systemDesignScores.reduce((a, b) => a + b, 0) /
          systemDesignScores.length
        : 0;
    ...
    const overallFit = Math.round(
      (technicalFit + behavioralFit + systemDesignFit) / 3,
    );
    ...
    const successProbability = Math.min(
      100,
      Math.max(0, overallFit * (company.acceptanceRate / 100)),
    );
...
export const getCompanyRecommendations = async (userId) => {
  try {
    ...
    if (userInterviews.length === 0) {
      return {
        message:
          "Complete some interviews first to get personalized recommendations",
        recommendations: [],
      };
    }
    ...
    const recommendations = await Promise.all(
      companies.map(async (company) => {
        const fit = await calculateCompanyFit(
          user,
          company.name,
          userInterviews,
        );
        ...
        if (fit.gaps.technical <= 0)
          fit.strengths.push("Strong technical skills");
        else if (fit.gaps.technical > 15)
          fit.weaknesses.push("Technical skills need improvement");

        if (fit.gaps.behavioral <= 0)
          fit.strengths.push("Excellent communication");
        else if (fit.gaps.behavioral > 15)
          fit.weaknesses.push("Behavioral skills need work");

        if (fit.gaps.systemDesign <= 0)
          fit.strengths.push("Great system design skills");
        else if (fit.gaps.systemDesign > 15)
          fit.weaknesses.push("System design needs practice");

        // Add recommendations
        if (fit.fitScore >= 70) {
          fit.recommendations.push(
            `You're well-prepared for ${company.name}! Keep practicing.`,
          );
        } else {
...
export const getCompanyFitAnalysis = async (userId, companyName) => {
  try {
    ...
    const userInterviews = await Interview.find({ user: userId }).sort({
      createdAt: -1,
    });
    ...
    const improvement =
      companyInterviews.length >= 2
        ? companyInterviews[0].result?.overallScore -
          companyInterviews[companyInterviews.length - 1].result?.overallScore
        : 0;
