...
export const createInterviewCalendar = async (userId, calendarData) => {
  try {
    ...
    const prepDays = Math.ceil(
      (new Date(interviewDate) - new Date()) / (1000 * 60 * 60 * 24),
    );
...
export const getUserCalendars = async (userId, status = "active") => {
  try {
    ...
    const calendars = await InterviewCalendar.find(query).sort({
      interviewDate: 1,
    });
 ...
 * @returns {Object} Updated calendar
 */
export const completeDailyPractice = async (
  calendarId,
  practiceId,
  practicesDone,
) => {
