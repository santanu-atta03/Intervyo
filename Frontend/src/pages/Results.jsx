    ...
    return {
      results: {
        summary: {
          ...
          correctAnswers:
            questionEvaluations.filter((q) => q.score >= 8).length || 0,
          partialAnswers:
            questionEvaluations.filter((q) => q.score >= 5 && q.score < 8)
              .length || 0,
          incorrectAnswers:
            questionEvaluations.filter((q) => q.score < 5).length || 0,
        },

        categoryBreakdown: {
          technical: {
            score:
              (session?.technicalScore || feedback?.technicalScore || 0) * 10,
            questionsAnswered:
              questionEvaluations.filter((q) => q.category === "technical")
                .length || 0,
        ...
        detailedFeedback: {
          overallAssessment:
            feedback?.summary || session?.feedback?.summary || "",
          strengths: feedback?.strengths || session?.feedback?.strengths || [],
          weaknesses:
            feedback?.improvements || session?.feedback?.improvements || [],
          keyHighlights:
            feedback?.keyHighlights || session?.feedback?.keyHighlights || [],
          areasOfConcern:
        ...
        improvementPlan: {
          ...
          mediumTerm: generateImprovementGoals(
            feedback?.improvements,
            "medium",
          ),
          longTerm: generateImprovementGoals(feedback?.improvements, "long"),
          recommendedCourses: generateCourseRecommendations(
            session?.interviewId?.role,
          ),
          practiceResources: generatePracticeResources(
            session?.interviewId?.role,
          ),
        },
    ...
    return [
      {
        ...
        description:
          "Daily coding challenges to improve problem-solving skills",
      ...
      },
      {
        ...
        description:
          "Practice with peers or mentors for real interview experience",
        url: "#",
      },
    ];
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">
              {config.domain} {config.subDomain && `- ${config.subDomain}`} (
              {config.difficulty})
            </p>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "questions" && (
            ...
            >
              {questionAnalysis && (
                <QuestionAnalysis questions={questionAnalysis} />
              )}
            </motion.div>
          )}
